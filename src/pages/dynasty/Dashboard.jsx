...
export default function Dashboard() {
  ...
  const handlePlayersLeavingSave = async (playersLeaving) => {
    // Pure builder in src/api — the component only does I/O + feedback.
    const { updates, unmatchedRows } = buildPlayersLeavingSave(currentDynasty, playersLeaving)
    await updateDynasty(currentDynasty.id, updates)
  ...
  const handleTransferDestinationsSave = async (destinations) => {
    // Pure builder in src/api — the component only does I/O + feedback.
    const { updates, skippedRows } = buildTransferDestinationsSave(currentDynasty, destinations)
    await updateDynasty(currentDynasty.id, updates)
  ...
  const handlePortalTransferClassSave = async (classSelections) => {
    // Pure builder in src/api — the component only does I/O.
    const { updates } = buildPortalTransferClassSave(currentDynasty, classSelections)
    await updateDynasty(currentDynasty.id, updates)
  }
  ...
  const handleFringeCaseClassSave = async (classSelections) => {
    // Pure builder in src/api — the component only does I/O.
    const { updates } = buildFringeCaseClassSave(currentDynasty, classSelections)
    await updateDynasty(currentDynasty.id, updates)
  }
