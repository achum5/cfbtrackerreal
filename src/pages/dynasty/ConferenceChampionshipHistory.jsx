...
export default function ConferenceChampionshipHistory() {
  ...
  const structurallyDetectedCCGames = useMemo(() => {
    // PC (auto-synced) dynasties only. The heuristic below leans on the
    // whole-league slate a sync imports: "one isolated game at the latest
    // week" separates a championship from a final round-robin week ONLY
    // because that week normally holds several same-conference games.
    // A console/manual dynasty usually records just the user's own games,
    // so its conference has exactly one game in EVERY week — the isolation
    // test is satisfied trivially and an ordinary week-13/14 conference
    // finale would be fabricated into a championship that was never played.
    // The bug this fixes is in buildWholeLeagueGames, which is PC-only, so
    // there's nothing to fill in for a manual dynasty anyway: it enters its
    // conference championships by hand.
    if (!isPcAutoDynasty(currentDynasty)) return []
    ...
    return detected
  }, [currentDynasty, currentDynasty?.games, currentDynasty?.teams])
