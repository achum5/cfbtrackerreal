...
export function buildWeekRecapPrompt(dynasty, year, week, opts = {}) {
  ...
  return [
    ...
    isBowlWeek ? [
      ...
      `  1. LEAD SECTION — the week's biggest story, named for it. Use the most consequential result per LEDE HIERARCHY. A top-5 upset, ranked-vs-ranked heavyweight, or defending champion's result each deserve their own section name.`,
      `  2. THE REST OF THE RANKED LANDSCAPE — other ranked teams' results worth noting. Group routine wins together in one clause; spend sentences only on results with texture. Skip ranked teams with nothing interesting to say — don't march the poll in numerical order.`,
      ...
      `  4. AROUND THE COUNTRY — notable unranked games: upsets, blowouts ≥30, rivalry results with revenge context (LAST MEETINGS), conference stakes. Selective — one focused paragraph, not every game.`,
      `  Optional: a standout stat leader, conference-race standings snapshot, or a one-line what's-next (only when the latest poll is labeled "POST-${weekLabel.toUpperCase()}").`,
    ].join('\n'),
    ``,
