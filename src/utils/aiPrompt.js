...
export function buildAIPrompt({
  ...
  const sections = [
    ...
    `  For EVERY data row, confirm you can point to that exact item in a screenshot or video attached to THIS request. Delete any row you are reconstructing from memory, from earlier in the conversation, or from a previous week. If it is not in a screenshot or video in front of you right now, it does not belong in the output — this is the most common corruption, a stale row bleeding in from a prior week.`,
    ``,
    `Only after all checks pass do you send the reply.`,
    ``,
    `═══════════════════════════════════════════════════════════`,
    `SCOPE — ONLY THE SCREENSHOTS OR VIDEO IN THIS REQUEST`,
    `═══════════════════════════════════════════════════════════`,
    `Every reply covers ONLY the screenshots or video attached to the CURRENT`,
    `request. The user may attach still screenshots, a screen recording, or`,
    `both — read whichever they sent, and scrub through a video the same way`,
    `you would read a series of screenshots.`,
    ...
    `visible in a screenshot or video attached to THIS request, it does not go`,
    `in the output — even if you "remember" that player from before. Each`,
    `weekly run is independent: the only correct rows are the ones you can see`,
    `right now.`,
    ``,
    `MULTI-PART UPLOADS — if the user sends more screenshots or video next:`,
    `The user may hit an attachment limit and send additional screenshots or`,
    `video in a follow-up message with no new prompt (or only brief text like`,
    `"more", "continue", "here's the rest", or just the attachments alone).`,
    `When that happens:`,
    ``,
    `  CONTINUATION (default ONLY when no new prompt block is pasted): treat`,
    `  the new attachments as the rest of the SAME upload. Re-deliver the`,
    `  complete output for THIS request — the original screenshots/video PLUS`,
    `  the new ones. Do NOT repeat a row already output; just add the new ones.`,
    `  "Combined output" means combined across the attachments of THIS one`,
    ...
    `  new title or week), OR the attachments are clearly a different`,
    ...
    `  screenshots or video. NEVER merge a new week's data into a prior week's`,
    `  rows.`,
    ``,
    `For every row ask: "can I see this exact player in a screenshot or video`,
    `attached to the current request?" If no, drop it.`,
    ``,
    `CRITICAL — NEVER INVENT OR CARRY OVER MISSING DATA:`,
    `If the user tells you your output was incomplete or missed something`,
    `but sends NO new screenshots or video, do NOT fill in the gaps from memory`,
    `or inference, and do NOT pad the output with half-remembered rows from`,
    `earlier in the chat. Instead, reply with ONE line:`,
    `  "Please send the missing screenshots or video and I will add them."`,
    `Do not re-deliver a "completed" TSV unless you have actual screenshots or`,
    `video to read the missing data from. Inventing or carrying over rows that`,
    `look plausible is a data corruption error — the user pastes bad data.`,
    ``,
    `Sheet structure:`,
    structure.trim(),
  ...
  const rosterBlock = buildRosterBlock(roster, [
    ...
    '',
    'When the screenshot or video shows an abbreviated form (e.g. "A. Guess", "J. Smith",',
    ...
    'disambiguate.',
    '',
    'When the screenshot or video already shows a FULL name (e.g. "Kevin Applewhite",',
    ...
    'Never blank a cell just because the name is missing from this list when the',
    'screenshot or video itself shows the full name clearly.',
    '',
    'Only blank the cell when (a) the screenshot or video is illegible at that',
    'spot, OR (b) it shows an abbreviation AND no entry below resolves it',
    'unambiguously.',
    '',
  ].join('\n'))
  if (rosterBlock) {
  ...
  if (includeTeamMap) {
    ...
    sections.push(
      ...
      `• Copy the name character-for-character as written above. Match the team in the screenshot or video to this list by school; output that list entry verbatim.`,
      `• The two Miami schools are disambiguated: output "Miami (FL)" for the Hurricanes and "Miami (OH)" for the RedHawks. Use the logo/colors/conference in the screenshot or video to tell them apart.`,
      ...(hasAliases ? [
        `• Some teams show a DIFFERENT name in EA CFB, marked above as "(in-game name: …)". When a screenshot or video shows that in-game name, output the LIST NAME (the part BEFORE the parenthesis), never the in-game name. Example: an attachment showing "Louisiana" → output "Lafayette Ragin' Cajuns" (NOT "Louisiana", and NOT the similarly-named "Louisiana Tech" or "LSU").`,
      ] : []),
