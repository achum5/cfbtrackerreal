...
export async function migrateOrphanedSocialFeedDoc(dynastyId, games) {
  ...
  // to live. Two ways that isn't true, and BOTH have to block the delete:
  // a post left behind (its game no longer exists, so nothing else carries
  // it), or a recovery write that failed above — its posts still exist only
  // in the orphaned doc, so deleting it would destroy the very data this
  // migration exists to rescue. Leaving the doc in place just means the
  // next run retries; it is already idempotent (posts merge by id).
  if (leftBehind.length === 0 && !writeFailed) {
