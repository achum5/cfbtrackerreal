...
const state = {
  failRecoveryWrites: false, // make recovery batches reject, to test the clear guard
...
vi.mock('firebase/firestore', () => {
  ...
  const writeBatch = vi.fn(() => {
    let hasSet = false
    return {
      set: (ref, data) => { hasSet = true; state.writes.push({ path: ref.path, data }) },
      delete: (ref) => { state.deletes.push(ref.path) },
      update: () => {},
      // state.failRecoveryWrites makes only the recovery batches (the ones
      // carrying set()) reject, leaving the clear batch able to commit — so
      // a test can prove the clear is skipped by choice, not by collateral.
      commit: async () => {
        if (hasSet && state.failRecoveryWrites) throw Object.assign(new Error('write failed'), { code: 'unavailable' })
      },
    }
  })
...
beforeEach(() => {
  ...
  state.writes = []
  state.deletes = []
  state.failRecoveryWrites = false
})
...
// A recovery write that fails leaves those posts existing ONLY in the
// orphaned doc, so clearing it would destroy the very data this migration
// exists to rescue. A failed write has to block the delete exactly like a
// left-behind post does. Costs nothing to keep: posts merge by id, so the
// next run just retries.
describe('migrateOrphanedSocialFeedDoc — recovery write failure', () => {
  it('leaves the orphaned doc in place when the recovery write fails', async () => {
    state.failRecoveryWrites = true
    state.orphanedDocs = [{
      id: '2026-NaN',
      data: { year: 2026, week: null, posts: [{ id: 'p-bowl-1', gameId: 'game-bowl', text: 'bowl post' }] },
    }]
