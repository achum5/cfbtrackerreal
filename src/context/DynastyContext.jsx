...
export function DynastyProvider({ children }) {
  ...
  const updateDynasty = async (dynastyId, updates, options = {}) => {
    ...
    try {
      ...
      // that team. Same walk as the full-map handler above, and likewise
      // cloned rather than mutated (the value is shared with
      // updatesWithTimestamp, which seeds local React state).
      {
        ...
        for (const key of Object.keys(mainDocUpdates)) {
          ...
          const byYear = team?.byYear
          if (!team || typeof team !== 'object' || !byYear || typeof byYear !== 'object') continue
          let byYearTouched = false
          const nextByYear = { ...byYear }
          for (const [yearKey, yearData] of Object.entries(byYear)) {
            if (!yearData || typeof yearData !== 'object') continue
          ...
          if (byYearTouched) mainDocUpdates[key] = { ...team, byYear: nextByYear }
        }
      }
      {
